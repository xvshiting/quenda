...
from kora.host.identity import DefaultUserResolver, User
...
from kora.host.registry import LoadedToolCatalog, NamedToolSpec, ToolRegistryBuilder
...
if TYPE_CHECKING:
    from kora.host.commands import CommandRegistry
    from kora.host.compression_policy import CompressionPolicy
    from kora.host.interactions import InteractionRegistry
    from kora.host.skill import SkillPackage
    ...
    from kora.tools.execution import SandboxConfig
...
def _resolve_tools(
    ...
    **Compatibility default**: If tools.bundles is missing or empty,
    ...
    if "network" in requested_bundles:
        from kora.tools.network import HTTPRequestTool, WebFetchTool, WebSearchTool
...
def _resolve_sandbox_config(config: AgentConfigYaml | None) -> SandboxConfig:
    ...
    """
    from kora.tools.execution import SandboxConfig
    from kora.tools.security.patterns import SANDBOX_ALLOWED_MODULES
...
def setup_agent(
    ...
    *,
    ...
    1. Resolve workspace binding (.kora/workspace.yaml)
