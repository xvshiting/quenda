...
1. User-workspace skills: ~/.kora/users/<user>/workspaces/<ws_id>/skills/ (highest priority)
2. Agent-package bundled: <agent_package>/skills/
3. User-level: ~/.kora/skills/
"""
...
class SkillDiscovery:
    ...
    - User-workspace: ~/.kora/users/<user>/workspaces/<ws_id>/skills/
      (user-specific skills for a workspace, highest priority)
    - Agent package: <agent_package>/skills/ (bundled with agent)
    - User: ~/.kora/skills/ (shared across workspaces, lowest priority)
    """
    ...
    def __init__(
        ...
        Args:
            user_workspace_skills_path: Path to user-workspace skills directory
                (e.g., ~/.kora/users/<user>/workspaces/<ws_id>/skills/).
    ...
    def _skill_directories(self) -> list[Path]:
        ...
        Priority order:
        1. User-workspace: ~/.kora/users/<user>/workspaces/<ws_id>/skills/ (highest)
        2. Agent package: <agent_package>/skills/ (bundled skills)
        3. User: ~/.kora/skills/ (shared, lowest priority)
        """
        dirs: list[Path] = []
        ...
        dirs.append(Path.home() / ".kora" / "skills")
    ...
    def _parse_skill(self, skill_dir: Path, skill_file: Path) -> SkillPackage | None:
        ...
        try:
            ...
            commands: list[str] = []
            if frontmatter.kora:
                for trigger in frontmatter.kora.activates_on:
                    if trigger.command:
    ...
    def _resolve_resources(
        ...
        if frontmatter.kora is None:
            return resources

        catalog = frontmatter.kora.resources
    ...
    def _determine_source(self, skill_dir: Path) -> Literal["user_workspace", "agent_package", "user", "system"]:
        ...
        # Check user level
        try:
            skill_dir.relative_to(Path.home() / ".kora")
            return "user"
        except ValueError:
            pass
