...
if TYPE_CHECKING:
    ...
    from kora.host.skill.package import SkillPackage
...
1. **User-workspace skills** - `~/.kora/users/<user>/workspaces/<ws_id>/skills/<skill-name>/SKILL.md`
...
3. **User skills** - `~/.kora/skills/<skill-name>/SKILL.md`
...
version: "1.0.0"
kora:                           # Optional Kora-specific config
  activates_on:
...
class InstructionScope(IntEnum):
    ...
    USER_GLOBAL = 3     # ~/.kora/users/<user>/INSTRUCTIONS.md
    USER_AGENT = 4      # ~/.kora/users/<user>/agents/<agent>/INSTRUCTIONS.md
    WORKSPACE = 5       # <workspace>/.kora/INSTRUCTIONS.md
    WORKSPACE_AGENT = 6 # <workspace>/.kora/agents/<agent>/INSTRUCTIONS.md
...
def resolve_instruction_sources(
    ...
    user_agent = Path.home() / ".kora" / "users" / user.id / "agents" / agent_name / "INSTRUCTIONS.md"
    ...
    # 6. Workspace INSTRUCTIONS.md
    workspace_instructions = workspace_path / ".kora" / "INSTRUCTIONS.md"
    ...
    # 7. Workspace-agent INSTRUCTIONS.md
    workspace_agent = workspace_path / ".kora" / "agents" / agent_name / "INSTRUCTIONS.md"
